...
class Config(object):
    ...
    def __init__(self, dataset):
        ...
        self.save_path = dataset + '/saved_dict/'+ self.model_name + '_lstm_食物'+'.ckpt'        # 模型训练结果
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')   # 设备

        self.require_improvement = 400                                 # 若超过1000batch效果还没提升，则提前结束训练
...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.norm = nn.LayerNorm(config.rnn_hidden)  # 归一化，注意这里的维度调整
    ...
    def forward(self, x):
        ...
        hidden_last_out = hidden_last[-1]  # 只使用最后一个隐藏状态
        # hidden_last_out = self.norm(hidden_last_out)
